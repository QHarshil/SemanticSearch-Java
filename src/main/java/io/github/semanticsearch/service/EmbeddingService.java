 ...
 * Includes caching, retry, and circuit breaker patterns.
 */
@Service
...
public class EmbeddingService {
    ...
    public List<Float> embed(String text) {
        ...
        log.debug("Successfully generated embedding with {} dimensions", embeddings.get(0).getEmbedding().size());
        return embeddings.get(0).getEmbedding();
    }

    /**
     * Fallback method for embedding generation when OpenAI API fails.
     * Returns an empty list as fallback.
     *
     * @param text Text that was being embedded
     * @param e Exception that triggered the fallback
     * @return Empty list as fallback
     */
    private List<Float> fallbackEmbed(String text, Exception e) {
