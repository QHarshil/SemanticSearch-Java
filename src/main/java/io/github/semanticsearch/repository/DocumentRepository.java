...
public interface DocumentRepository extends JpaRepository<Document, UUID> {
     ...
     * @return Page of documents
     */
    @Query("SELECT d FROM Document d WHERE LOWER(d.title) LIKE LOWER(CONCAT('%', :text, '%')) OR LOWER(d.content) LIKE LOWER(CONCAT('%', :text, '%'))")
    Page<Document> findByTitleOrContentContainingIgnoreCase(@Param("text") String text, Pageable pageable);
